...
def check_if_index_exists(index_name: str, region: str, host: str, auth: AWSV4SignerAuth) -> OpenSearch:
...
def process_shard(shard, embeddings_model_endpoint_name, aws_region, os_index_name, os_domain_ep) -> int:
...
if __name__ == "__main__":
    ...
    # get AWSV4SignerAuth to talk to opensearch
    http_auth = get_auth(args.aws_region)
    ...
    with mp.Pool(processes = args.process_count) as pool:
        results = pool.map(task, shards[shard_start_index:])
