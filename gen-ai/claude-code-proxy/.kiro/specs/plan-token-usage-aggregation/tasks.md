...
- [x] 1. 데이터베이스 스키마 변경
  - [x] 1.1 usage_aggregates 테이블에 provider 컬럼 추가 마이그레이션 생성
  ...
  - [x] 1.2 UsageAggregateModel에 provider 컬럼 추가
    - `backend/src/db/models.py` 수정
    - provider 필드 및 테이블 인덱스 정의 업데이트
    - _Requirements: 3.1_
  
  - [x] 1.3 UsageAggregate 도메인 엔티티에 provider 필드 추가
    - `backend/src/domain/entities.py` 수정
    - _Requirements: 3.1_

- [x] 2. Checkpoint - 마이그레이션 검증
  - 마이그레이션 적용 후 기존 데이터가 provider="bedrock"으로 설정되었는지 확인
  - 테스트 실행하여 기존 기능 정상 동작 확인

- [x] 3. PricingConfig 프로바이더 지원 확장
  - [x] 3.1 PricingConfig에 provider 파라미터 추가
...
- [x] 8. Plan 스트리밍 사용량 기록 통합
  ...
  - [x] 8.2 _build_plan_streaming_response 헬퍼 함수 생성
    - Bedrock 스트리밍과 유사한 구조로 Plan 스트리밍 응답 빌더 생성
    - 사용량 수집 및 기록 로직 포함
    - _Requirements: 2.3, 2.4_
  
  - [x] 8.3 SSE 이벤트 파싱 속성 테스트 작성
...
- [x] 10. Admin API 프로바이더 필터링 추가
  - [x] 10.1 사용량 조회 엔드포인트에 provider 쿼리 파라미터 추가
