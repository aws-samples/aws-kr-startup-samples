...
CREATE INDEX {table}_trans_datetime_key ON {database}.{schema}.{table}(trans_datetime);
'''

DROP_TABLE_SQL_FMT = '''DROP TABLE IF EXISTS {database}.{schema}.{table} CASCADE;'''

INSERT_SQL_FMT = '''INSERT INTO {database}.{schema}.{table} (customer_id, event, sku, amount, device, trans_datetime) VALUES('{customer_id}', '{event}', '{sku}', {amount}, '{device}', '{trans_datetime}');'''

DB_URL_FMT = 'postgresql+psycopg://{user}:{password}@{host}/{database}?autocommit=True'
...
def main():
  ...
  parser.add_argument('-u', '--user', action='store', default='postgres',
    help='user name (default: postgres)')
  ...
  parser.add_argument('--schema', action='store', default='public',
  ...
  if options.create_table:
    sql_stmt = CREATE_TABLE_SQL_FMT.format(database=options.database, schema=options.schema, table=options.table)
    print(sql_stmt)
    if not options.dry_run:
      db.query(sql_stmt)
    return
  ...
  if options.drop_table:
    sql_stmt = DROP_TABLE_SQL_FMT.format(database=options.database, schema=options.schema, table=options.table)
    print(sql_stmt)
    if not options.dry_run:
      db.query(sql_stmt)
    return
  ...
  for cnt in itertools.takewhile(predicate, itertools.count()):
    ...
    sql_stmt = INSERT_SQL_FMT.format(database=options.database, schema=options.schema, table=options.table, **json_record)
