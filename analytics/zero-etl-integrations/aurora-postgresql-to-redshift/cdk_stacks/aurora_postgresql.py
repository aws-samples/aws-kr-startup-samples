...
class AuroraPostgresqlStack(Stack):
  ...
  def __init__(self, scope: Construct, construct_id: str, vpc, **kwargs) -> None:
    ...
    db_cluster_name = self.node.try_get_context('rds_cluster_name')

    self.sg_postgresql_client = aws_ec2.SecurityGroup(self, 'PostgreSQLClientSG',
      vpc=vpc,
      allow_all_outbound=True,
      description='security group for postgresql client',
      security_group_name=f'{db_cluster_name}-postgresql-client-sg'
    )
    cdk.Tags.of(self.sg_postgresql_client).add('Name', 'postgresql-client-sg')
    ...
    sg_postgresql_server.add_ingress_rule(peer=self.sg_postgresql_client, connection=aws_ec2.Port.tcp(5432),
    ...
    rds_engine = aws_rds.DatabaseClusterEngine.aurora_postgres(version=aws_rds.AuroraPostgresEngineVersion.VER_16_6)
    ...
    rds_cluster_param_group = aws_rds.ParameterGroup(self, 'AuroraPostgreSQLClusterParamGroup',
      engine=rds_engine,
      description='Custom cluster parameter group for aurora-postgresql16',
      name=f"{db_cluster_name}-cluster-pg",
      parameters={
        'log_min_duration_statement': '15000', # 15 sec
        'default_transaction_isolation': 'read committed',
        'client_encoding': 'UTF8',
        #XXX: Setting up Aurora PostgreSQL
        # https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/zero-etl.setting-up.html#zero-etl.parameters
    ...
    rds_db_param_group = aws_rds.ParameterGroup(self, 'AuroraPostgreSQLDBParamGroup',
      engine=rds_engine,
      description='Custom parameter group for aurora-postgresql16',
      name=f"{db_cluster_name}-pg",
      parameters={
        'log_min_duration_statement': '15000', # 15 sec
        'default_transaction_isolation': 'read committed'
      }
    ...
    self.rds_cluster = aws_rds.DatabaseCluster(self, 'Database',
      engine=rds_engine,
    ...
    cdk.CfnOutput(self, 'DBClusterReadEndpoint',
      value=self.rds_cluster.cluster_read_endpoint.socket_address,
      export_name=f'{self.stack_name}-DBClusterReadEndpoint')
    cdk.CfnOutput(self, 'RDSClientSecurityGroupId',
      value=self.sg_postgresql_client.security_group_id,
