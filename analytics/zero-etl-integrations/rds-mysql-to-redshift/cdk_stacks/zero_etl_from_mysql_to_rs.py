...
class ZeroEtlFromMysqlToRedshifStack(Stack):

  def __init__(self, scope: Construct, construct_id: str, rds_instance, rss_namespace, **kwargs) -> None:
    super().__init__(scope, construct_id, **kwargs)

    zero_etl_integration_setting = self.node.try_get_context('zero_etl_integration') or {}

    INTEGRATION_NAME = f"{rds_instance.instance_identifier}-to-{rss_namespace.attr_namespace_namespace_name}"[:63]
    ...
    cfn_integration = aws_rds.CfnIntegration(self, "ZeroETLIntegration",
      source_arn=rds_instance.instance_arn,
