# Hosting Video-LLaVa model on Amazon SageMaker Real-time Inference Endpoint using SageMaker HuggingFace DLC

This is a CDK Python project to host the [Video-LLaVA](https://github.com/PKU-YuanGroup/Video-LLaVA) model
on Amazon SageMaker Real-time Inference Endpoint.

[Video-LLaVA](https://huggingface.co/docs/transformers/model_doc/video_llava) is is an open-source multimodal LLM trained by fine-tuning LlamA/Vicuna on multimodal instruction-following data generated by Llava1.5 and VideChat.
...
$ git sparse-checkout set machine-learning/sagemaker/video-llava-on-aws-sagemaker
$ cd machine-learning/sagemaker/video-llava-on-aws-sagemaker
...
Following [deploy_video_llava_on_sagemaker_realtime_endpoint.ipynb](src/notebook/deploy_video_llava_on_sagemaker_realtime_endpoint.ipynb) on the SageMaker Studio, we can deploy the model to Amazon SageMaker.

## Example

Following [video-llava_realtime_endpoint.ipynb](src/notebook/video-llava_realtime_endpoint.ipynb) on the SageMaker Studio, we can invoke the model with sample data.
