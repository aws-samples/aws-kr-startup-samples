...
def get_vector_store():
    ...
    # Initial documents
    initial_docs = [
        Document(page_content="Amazon Web Services (AWS) is a comprehensive cloud computing platform.", metadata={}),
        Document(page_content="Machine Learning algorithms learn patterns from data automatically.", metadata={}),
        Document(page_content="Kubernetes orchestrates containerized applications across clusters.", metadata={}),
        Document(page_content="Vector databases store high-dimensional vectors for AI applications.", metadata={}),
        Document(page_content="Python is a versatile programming language for data science.", metadata={})
    ]
...
def add_document(vector_store, text):
...
def process_uploaded_file(uploaded_file, vector_store):
    ...
    try:
        ...
        if text_content and len(text_content.strip()) > 0:
            ...
            log_operation("process", f"Processed: {filename} ({len(text_content)} chars)", True)
                ...
                with st.form("add_document_form", clear_on_submit=True):
                    ...
                    if submitted and new_text:
                        with st.spinner("Adding document..."):
                            success, message = add_document(st.session_state.vector_store, new_text)
                            if success:
                                st.success(message)
                                st.rerun()
                            else:
            ...
            with tab2:
                ...
                if uploaded_file and st.button("Upload & Process"):
                    with st.spinner("Processing file..."):
                        success, message = process_uploaded_file(uploaded_file, st.session_state.vector_store)
                        if success:
                            st.success(message)
                            st.rerun()
                        else:
...
# Footer
st.markdown("---")
st.markdown("**Milvus on EKS Workshop** | Built with Streamlit & PyMuPDF")