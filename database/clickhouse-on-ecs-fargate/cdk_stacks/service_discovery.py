...
class ServiceDiscoveryStack(Stack):
  ...
  def __init__(self, scope: Construct, construct_id: str, vpc, **kwargs) -> None:
    ...
    private_dns_namespace_name = self.node.try_get_context('private_dns_namespace_name') or "clickhouse.local"
    self.namespace = aws_servicediscovery.PrivateDnsNamespace(self, "PrivateDnsNamespace",
      name=private_dns_namespace_name,
      description="Clickhouse Service Discovery namespace",
      vpc=vpc
    )
    cdk.Tags.of(self.namespace).add('Name', 'clickhouse')
    ...
    cdk.Tags.of(self.service).add('Name', 'clickhouse')
